...
<body>
	<div id="menu">
		<li class="list-group-item"
			...
			DANH MỤC</li> <a href="homeGV" class="list-group-item">Trang Chủ</a> <a
			href="infoGV" class="list-group-item ">Quản trị tài khoản</a> <a
			href="dkdt" class="list-group-item">Đăng kí đề tài</a><a
		...
		<c:if test="${sessionScope.acc.role == 4}">
			...
			<a href="listPB" class="list-group-item active">Phân giáo viên
				phản biện</a>
		</c:if>
		<hr />
	</div>
				...
				<div class="row">
							...
							<div id="desktop-menu">
								<div class="panel panel-default">
									...
									<a href="dkdt" class="list-group-item">Đăng kí đề tài</a><a
										href="listDTHD" class="list-group-item ">Quản lý nhóm</a>
									<c:if test="${sessionScope.acc.role == 4}">
										<a href="listduyetDT" class="list-group-item ">Duyệt đề
											tài</a>
										<a href="listPB" class="list-group-item active">Phân giáo
											viên phản biện</a>
									</c:if>
								</div>
							</div>
					...
					<div class="col-md-9">
						...
						<div class="panel panel-default">
							<div style="color: rgb(39, 86, 156);" class="panel-heading">
								<h4
									...
									Phân công giáo viên phản biện</h4>
							</div>
						</div>

						<div class="user-details">
							<div class="input-box">
								<span class="details">Tên đề tài</span> <select name=""
									id="detai"><option value="1">${detail.getDT(detail.detai).tenDT}</option>
								</select>
							</div>
						</div>
											...
											<tbody>
												<c:forEach items="${gv}" var="o">
													<tr>
														...
														<td><a href="addGVvaoHD?id=${detail.id}&&uid=${o.uId}"><button class="Add">Thêm</button></a>
														</td>
													</tr>
												</c:forEach>
											</tbody>
						...
						<div class="recent2-grid">
							<div class="projects">
								<div class="card-header">
									<h3>Danh sách giảng viên phản biện</h3>
								...
								<div class="card-body">
									<div class="table-responsive">
										<table width="100%">
											...
											<tbody>
												<c:forEach items="${gvpb}" var="p">
													<tr>

														<td>${p.fullName}</td>
														<td>${p.uId}</td>
														<td>${detail.getDT(detail.detai).tenDT}</td>
														<td><a href="deleteGVkhoiHD?id=${detail.id}&&uid=${p.uId}"><button class="Add">Xóa</button></td>
